...
namespace VirtualHealthProject.Controllers
{
    ...
    public class PatientFolderController : Controller
    {
        ...
        {
            var vm = new PatientManagementVM();
            vm = new PatientManagementVM
            {
                patientCount = GetPatient(),
                bedCount = GetBed(),
                admissionCount = GetAdmission(),
                occupiedbedsCount = GetOccupiedBeds()
            };
        ...
        {
            List<Patient> TempData2 = _context.Patients.Where(p => p.PatientID != null).ToList();
            int count = TempData2.Count();
            return count;
        }
        private int GetBed()
        {
            List<Bed> TempData2 = _context.Beds.Where(p => p.BedID != null).ToList();
            int count = TempData2.Count();
            return count;
        }
        private int GetOccupiedBeds()
        {
            List<BedAllocation> TempData2 = _context.BedAllocations
                .Where(p => p.AllocationID != null ).ToList();
            int count = TempData2.Count();
            return count;
        }
        private int GetAdmission()
        {
            List<Admission> TempData2 = _context.Admissions.Where(p => p.AdmissionId != null).ToList();
            int count = TempData2.Count();
            return count;
        }
